...
io.on("connection", (socket) => {
    ...
    // Video call events
    socket.on(socket_1.SocketEvent.VIDEO_CALL_STARTED, ({ roomId, callerUsername }) => {
        // Notify all users in the room that a call has started
        socket.broadcast.to(roomId).emit(socket_1.SocketEvent.VIDEO_CALL_STARTED, {
            roomId,
            callerUsername
        });
    });
    socket.on(socket_1.SocketEvent.VIDEO_CALL_REQUEST, ({ roomId, callerUsername }) => {
        // Send call request to all users in the room except the caller
        socket.broadcast.to(roomId).emit(socket_1.SocketEvent.VIDEO_CALL_REQUEST, {
            roomId,
            callerUsername
        });
    });
    socket.on(socket_1.SocketEvent.VIDEO_CALL_ACCEPTED, ({ roomId, username }) => {
        // Notify the caller that the call was accepted
        const users = getUsersInRoom(roomId);
        const caller = users.find(user => user.username === username);
        if (caller) {
            io.to(caller.socketId).emit(socket_1.SocketEvent.VIDEO_CALL_ACCEPTED, { username });
        }
    });
    socket.on(socket_1.SocketEvent.VIDEO_CALL_REJECTED, ({ roomId }) => {
        // Notify all users in the room that the call was rejected
        socket.broadcast.to(roomId).emit(socket_1.SocketEvent.VIDEO_CALL_REJECTED, { roomId });
    });
    socket.on(socket_1.SocketEvent.VIDEO_CALL_ENDED, ({ roomId }) => {
        // Notify all users in the room that the call has ended
        socket.broadcast.to(roomId).emit(socket_1.SocketEvent.VIDEO_CALL_ENDED, { roomId });
    });
